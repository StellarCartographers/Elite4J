...
public class GameFiles
{
    ...
    public static GameFiles getInstance(File directory)
    {
        if (GameFiles.instance == null || !GameFiles.instance.directory.equals(directory))
        {
            ...
            {
                throw new RuntimeException("Journal File not found: " + directory.getAbsolutePath());
            }
        }
    ...
    private File getGameFile(String namePrefix)
    {
        ...
        if (edFiles != null)
        {
            for (File file : edFiles)
            {
                if (!file.isDirectory())
                {
                    if (file.getName().startsWith(namePrefix) && GameFiles.GAME_FILE_EXTENSION.equals(getFileExtension(file)))
                    {
                        gameFile = file;
                    }
                }
    ...
    private static String getFileNameExtension(String filename)
    {
        ...
        if (filename != null)
        {
            extension = Optional.of(filename).filter(f -> f.contains(".")).map(f -> f.substring(filename.lastIndexOf(".") + 1)).orElse(null);
        }
        return extension;
    }
}
